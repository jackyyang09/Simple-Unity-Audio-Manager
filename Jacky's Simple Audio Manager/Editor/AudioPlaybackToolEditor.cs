...
namespace JSAM
{
    ...
    public class AudioPlaybackToolEditor : EditorWindow
    {
        ...
        public void DrawPlaybackTool(AudioClip selectedClip, AudioFileObject selectedSound = null, AudioFileMusicObject selectedMusic = null)
        {
            ...
            if (GUILayout.Button(buttonIcon, new GUILayoutOption[] { GUILayout.MaxHeight(20) }))
            {
                ...
                if (clipPlaying)
                {
                    ...
                    if (selectedSound)
                    {
                        helperHelper.PlayDebug(selectedSound, mouseScrubbed);
                        AudioFileObjectEditor.instance.StartFading(selectedSound);
                    }
                    else if (selectedMusic)
                    {
            ...
            if (selectedSound)
            {
                using (new EditorGUI.DisabledScope(selectedSound.GetFileCount() < 2))
                {
                    ...
                    {
                        AudioFileObjectEditor.instance.DesignateRandomAudioClip(selectedSound);
                        clipPlaying = true;
                        helperSource.Stop();
                        AudioFileObjectEditor.instance.StartFading(selectedSound);
                    }
                    ...
                    case EventType.MouseDrag:
                        if (evt.button == 0) // Left Mouse Events
                        {
                            ...
                            if (selectedMusic)
                            {
                                if (selectedMusic.loopMode == LoopMode.LoopWithLoopPoints && selectedMusic.clampToLoopPoints)
                                {
                                    helperSource.time = Mathf.Clamp(helperSource.time, selectedMusic.loopStart, selectedMusic.loopEnd - AudioManager.EPSILON);
                                }
                            }
                        }
            ...
            if (selectedSound || (selectedClip && !selectedMusic))
            {
                if ((clipPlaying && !clipPaused) || (mouseDragging && clipPlaying))
                {
                    ...
                    if (selectedSound)
                    {
                        ...
                        helperSource.pitch = selectedSound.startingPitch;
                    }
        ...
        public static Texture2D PaintWaveformSpectrum(AudioClip audio, int width, int height, Color col)
        {
            ...
            for (int i = leftSide; i < rightSide; i += packSize)
            {
